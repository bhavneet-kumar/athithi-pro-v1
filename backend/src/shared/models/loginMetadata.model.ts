...
LoginMetadataSchema.methods.generateEmailVerificationToken = async function () {
    ...
    this.emailVerificationExpires =
        new Date(Date.now() + TWENTY_FOUR_HOURS_IN_MILLISECONDS);
    await this.save();
};
...
LoginMetadataSchema.methods.generatePasswordResetToken = async function () {
    ...
    this.passwordResetExpires =
        new Date(Date.now() + THIRTY_MINUTES_IN_MILLISECONDS);
    await this.save();
};

export const LoginMetadata = model<ILoginMetadata>(
    'LoginMetadata',
    LoginMetadataSchema
);
